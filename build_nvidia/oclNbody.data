...
#define SX_SUM(i,j) sharedPos[i + MUL_24((uint)get_local_size(0), (uint)j)]    // i + blockDimx * j
                           ...
                           __local REAL4* sharedPos)
{
    ...
    unsigned int numTiles = numBodies / MUL_24(blockDimx, blockDimy);

    for (unsigned int tile = blockIdxy; tile < numTiles + blockIdxy; tile++) 
    {
        sharedPos[threadIdxx + blockDimx * threadIdxy] = 
            positions[WRAP(blockIdxx + MUL_24(blockDimy, tile) + threadIdxy, gridDimx) * blockDimx
                      + threadIdxx];
                             ...
                             __local REAL4* sharedPos)
{
    ...
    for (unsigned int tile = blockIdxy; tile < numTiles + blockIdxy; tile++) 
    {
        sharedPos[threadIdxx + MUL_24(blockDimx, threadIdxy)] = 
            ...
            __local REAL4* sharedPos)
{
    ...
    unsigned int index = MUL_24(blockIdxx, blockDimx) + threadIdxx;
