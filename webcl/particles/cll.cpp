...
CL::CL(int use_gpu)
{
    ...
    // an example is avaible in the NVIDIA code
        ...
        try{
            ...
            try{
                context = cl::Context(use_gpu?CL_DEVICE_TYPE_GPU:CL_DEVICE_TYPE_CPU, props);
            }
            catch (cl::Error er) {
                printf("ERROR: %s(%s)\n", er.what(), oclErrorString(er.err()));
            }
...
void CL::loadProgram(std::string path)
{
    //Program Setup
    int pl;
    ...
    std::string kernel_source = file_contents(path.c_str(), &pl);
    //printf("file: %s\n", cSourceCL);
    int program_length = (size_t)pl;
