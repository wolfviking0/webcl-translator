...
void performQueries() {
	cl_int errNum;
	printf("Performing queries on OpenGL objects:\n");
	...
	if( errNum != CL_SUCCESS ) {
		printf("Failed to get object information\n");
	} else {
		if( obj_type == CL_GL_OBJECT_TEXTURE2D ) {
			printf("Queried a texture object succesfully.\n");
			printf("Object name is: %d\n",objname); 
		}
	...
	if( errNum != CL_SUCCESS ) {
		printf("Failed to get texture information\n");
	} else {
		...
		if( param == GL_TEXTURE_RECTANGLE_ARB ) {
			printf("Texture rectangle ARB is being used.\n");
		}
	}
}
...
bool CreateMemObjects(cl_context context, GLuint texture, GLuint vbo, cl_mem *p_cl_vbo_mem, cl_mem *p_cl_tex_mem)
{
	...
	*p_cl_vbo_mem = clCreateFromGLBuffer(context, CL_MEM_READ_WRITE, vbo, &errNum );
	if( errNum != CL_SUCCESS )
	{
		printf("Failed creating memory from GL buffer.\n");
		return false;
	}
	...
	*p_cl_tex_mem = clCreateFromGLTexture2D(context, CL_MEM_READ_WRITE, GL_TEXTURE_RECTANGLE_ARB, 0, texture, &errNum );
	if( errNum != CL_SUCCESS )
	{
		printf("Failed creating memory from GL texture.\n");
		return false;
	}
...
int main(int argc, char** argv)
{
    ...
    program = CreateProgram(context, device, "hello_world_interop.cl");
    if (program == NULL)
    {
        Cleanup();
        return 1;
